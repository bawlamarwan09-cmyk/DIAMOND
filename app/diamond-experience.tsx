    ...
    <div className="hero-stage" ref={stage}>
      ...
      <div className="hero-content wrap">
        ...
        <div className="hero-actions entrance e4">
          <a className="button button-gold" href="#bridal">EXPLORE BRIDAL <ArrowRight size={18} /></a>
  ...
  return <div className={motion ? "site motion-on" : "site motion-off"}>
    ...
    <header className={`site-header ${headerSolid ? "solid" : ""}`}>
      <div className="header-inner wrap">
        ...
        <nav className="desktop-nav" aria-label="Main navigation"><a href="#bridal">BRIDAL</a><a href="#collections">THE RINGS</a><a href="#wedding-day">THE WEDDING DAY</a><a href="#craftsmanship">OUR CRAFT</a><a href="#bespoke">BESPOKE</a></nav>
        <div className="header-actions"><button className="motion-toggle" onClick={() => setMotion(v => !v)} aria-label={motion ? "Pause motion" : "Enable motion"} aria-pressed={motion}>{motion ? <Pause size={14} /> : <Play size={14} />}<span>MOTION {motion ? "ON" : "OFF"}</span></button><a className="header-cta" href="#collections">FIND YOUR FOREVER <ArrowUpRight size={14} /></a><button className="mobile-toggle" aria-label="Open navigation" onClick={() => setMenu(true)}><Menu size={24} /></button></div>
      </div>
    </header>
    <main>
      ...
      <section className="bridal-paths" aria-label="Explore bridal jewelry"><div className="wrap bridal-path-grid"><a className="bridal-path reveal" href="/product/solitaire-ring"><Image src="/media/solitaire.webp" alt="Solitaire diamond engagement ring" fill sizes="(max-width: 700px) 100vw, 33vw" className="cover" /><span className="bridal-path-copy"><small>01 / THE QUESTION</small><strong>Engagement Rings</strong><span>DISCOVER THE SOLITAIRE <ArrowRight size={17}/></span></span></a><a className="bridal-path reveal" href="#collections"><Image src="/media/bands.webp" alt="Wedding band design inspiration" fill sizes="(max-width: 700px) 100vw, 33vw" className="cover" /><span className="bridal-path-copy"><small>02 / THE PROMISE</small><strong>Wedding Bands</strong><span>EXPLORE THE COLLECTION <ArrowRight size={17}/></span></span></a><a className="bridal-path reveal" href="#wedding-day"><Image src="/media/wedding-vows.webp" alt="Couple exchanging rings at their wedding ceremony" fill sizes="(max-width: 700px) 100vw, 33vw" className="cover" /><span className="bridal-path-copy"><small>03 / THE CELEBRATION</small><strong>The Wedding Day</strong><span>STEP INTO THE MOMENT <ArrowRight size={17}/></span></span></a></div></section>
      ...
      <section id="collections" className="collections-section section-pad">
        <div className="wrap">
          ...
          <div className="ring-grid">
            {rings.map((r, i) => <button key={r.name} className="ring-card reveal" onClick={() => { if (i === 0) window.location.href = "/product/solitaire-ring"; else setSelectedRing(i); }} aria-label={i === 0 ? "Open The Solitaire product page" : r.detail} style={{ transitionDelay: `${i * 70}ms` }}>
          ...
          <div className="collection-foot"><span><Gem size={17} /> LAB-GROWN. ENDLESSLY CAPTIVATING.</span><button className="text-link" onClick={() => openFilm(4)}>STEP INSIDE THE COLLECTION <ArrowRight size={17} /></button></div>
        </div>
      </section>
    ...
    <footer className="site-footer"><div className="wrap"><div className="footer-top"><a className="wordmark" href="#home"><span>LAB GRANT <span className="wordmark-diamond">DIAMOND</span></span><small>A BRIGHTER KIND OF FOREVER</small></a><nav aria-label="Footer navigation"><a href="#bridal">BRIDAL</a><a href="#collections">THE COLLECTION</a><a href="#wedding-day">THE WEDDING DAY</a><a href="#bespoke">BESPOKE</a><a href="#questions">QUESTIONS</a><button onClick={() => openFilm()}>THE FILMS</button></nav><a href="#home" className="back-top" aria-label="Back to top"><ArrowUpRight size={20} /></a></div><div className="footer-bottom"><span>© {new Date().getUTCFullYear()} Lab Grant Diamond.</span><span>MODERN LOVE. TIMELESS BRILLIANCE.</span></div></div></footer>
    <Dialog open={menu} onOpenChange={setMenu}><DialogContent className="navigation-dialog"><DialogTitle className="sr-only">Navigation</DialogTitle><DialogDescription className="sr-only">Explore Lab Grant Diamond</DialogDescription><span className="eyebrow">LAB GRANT DIAMOND</span><nav aria-label="Mobile navigation">{[["Bridal", "bridal"], ["The rings", "collections"], ["The wedding day", "wedding-day"], ["Our craft", "craftsmanship"], ["Bespoke", "bespoke"], ["Questions", "questions"]].map(([title,id],i)=><a key={id} href={`#${id}`} onClick={()=>setMenu(false)}><span>0{i+1}</span>{title}<ArrowUpRight size={22}/></a>)}</nav><button className="text-link" onClick={()=>{setMenu(false);openFilm();}}>WATCH THE FILM <Play size={14}/></button></DialogContent></Dialog>
